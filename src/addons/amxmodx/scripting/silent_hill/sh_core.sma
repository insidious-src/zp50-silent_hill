...
{
	if (gMode >= 0 && !is_user_bot (id))
        set_task (START_TIME, "display_gamemode", id, "", 0, "b");
}
...
public display_gamemode (id)
{
	...
	{
		// Hud Options
		set_hudmessage (rgb_hud_colors[gMode][0],
                        rgb_hud_colors[gMode][1],
                        rgb_hud_colors[gMode][2],
						HUD_MODE_X, HUD_MODE_Y,
                        0, 6.0, 12.0, 0.5, 0.6);

		// Now the hud appears
		ShowSyncHudMsg (id, gHudSync, "%s: %s", "Active Mode", ModeName);
	}
}
